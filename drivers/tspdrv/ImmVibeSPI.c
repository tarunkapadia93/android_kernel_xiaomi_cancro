...
static void drv2604_write_reg_val(const unsigned char* data, unsigned int size)
{
	...
	if(g_hw_version == 5) {
		while (i < size) {
			/* From Xiaomi start */
			//printk("drv2604 x5 write 0x%02x, 0x%02x", data[i], data[i + 1]);
			/* From Xiaomi end */
			if(data[i] == 0x02)
		...
		while (i < size) {
			/* From Xiaomi start */
			//printk("drv2604 write 0x%02x, 0x%02x", data[i], data[i + 1]);
			/* From Xiaomi end */
			i2c_smbus_write_byte_data(g_pTheClient, data[i], data[i + 1]);
			i += 2;
		}
...
//	return i2c_smbus_read_byte_data(g_pTheClient, reg);
	...
	//printk("drv2604 read addr:0x%x reg:0x%x data:0x%x res:%d", address, reg, data, res);
